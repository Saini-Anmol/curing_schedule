"""
JK Tyre BTP — PCR Curing MILP Scheduler v1
===============================================
===============================================
Full production-ready MILP-based monthly curing schedule generator.

This is an MILP (Mixed Integer Linear Programming) drop-in replacement for the
LP + post-hoc rounding approach. The solver returns integer cycle counts
directly, eliminating the floor-rounding loss that the LP variant suffers.

Architecture (same as LP variant — only the solver changes)
-----------------------------------------------------------
Phase 1 : ETL          — load & clean all input data
Phase 2 : MILP Solve   — globally optimal INTEGER cycle allocation
Phase 3 : Extract      — pull integer cycles + build run order (no rounding loss)
...
Objective:
    ...
    (slack dominates; eps is small, concentrates each SKU on fewer presses)

Constraints:
    Demand   :  CAVITIES * sum_m c[s,m] + slack[s] >= Demand[s]
    Capacity :  sum_s ct[s]*c[s,m] + co * sum_s y[s,m] <= avail[m] - locked[m] + co
                (encodes (n_active - 1) changeovers when n_active >= 1)
    Linking  :  c[s,m] <= BigM[s,m] * y[s,m]        (c > 0 forces y = 1)

Input / output format is IDENTICAL to the LP variant — same CSVs/XLSX in,
same 5-sheet Excel out. Only the algorithm differs.
        ...
        for mach, sku_order in machine_sku_order.items():
            ...
            for idx, sku in enumerate(sku_order):
                ...
                prod_rows, units_so_far = self._split_block(
                    ...
                    gt, "MILP Scheduled", units_so_far,
                )
...
# ORCHESTRATOR
# ══════════════════════════════════════════════════════════════════════════════
class JK_MILP_Curing_Scheduler_v1:
    def __init__(self):
        self.avail_mins = Config.avail_mins()
    ...
    def run(
        ...
        print("\n" + "="*64)
        print(f"  JK Tyre PCR Curing MILP Scheduler v1")
        ...
        print(f"  MILP limits : time={Config.MILP_TIME_LIMIT_SEC}s | "
        ...
        # Reduce MILP demand by what continuity already covers
        # SKUs fully handled in continuity are excluded from the MILP entirely
        df_lp = df_valid.copy()
        for sku, remainder in demand_remainder.items():
            mask = df_lp["SKUCode"] == sku
            if remainder == 0:
                # Continuity covers all demand — remove from MILP
                df_lp = df_lp[~mask]
            else:
                # Continuity covers part — MILP only needs the remainder
        ...
        print(f"  [MILP Input] {len(df_lp)} SKUs | "
        ...
        df_mach, machine_sku_order = extractor.extract(x, meta, df_lp, locked_mins)
...
class ExcelExporter:
    ...
    def export(self, results: dict):
        ...
        with pd.ExcelWriter(self.path, engine="openpyxl") as writer:
            ...
            self._title(ws,"PCR CURING MILP v1 — DEMAND FULFILLMENT",kpi,len(cols1))
            ...
            self._title(ws2,"MACHINE-WISE SCHEDULE — PCR MILP v1 (CO + Cleaning)",kpi,len(cols2))
            ...
            self._title(ws3,"SHIFT-WISE SCHEDULE — PCR MILP v1",kpi,len(cols3))
            ...
            self._title(ws4,"PRESS UTILIZATION — PCR MILP v1",
...
def run_from_excel(
    ...
    scheduler = JK_MILP_Curing_Scheduler_v1()
