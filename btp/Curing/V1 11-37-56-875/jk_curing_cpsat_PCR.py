"""
JK Tyre BTP — PCR Curing CP-SAT Scheduler v1
===============================================
===============================================
Full production-ready CP-SAT-based monthly curing schedule generator.

Uses Google OR-Tools' CP-SAT solver — a Constraint Programming solver with
SAT + LP relaxation and parallel search. Drop-in replacement for the LP +
rounding approach: same inputs, same 5-sheet Excel output, only the
allocation algorithm changes.

Architecture (same as LP variant; only Phase 2-3 differ)
--------------------------------------------------------
Phase 1 : ETL          — load & clean all input data
Phase 2 : CP-SAT Solve — globally optimal INTEGER allocation (parallel search)
Phase 3 : Extract      — pull integer cycles + build run order (no rounding)
Phase 4 : Schedule     — build shift-wise row-level schedule
Phase 5 : Export       — Excel output (same 5-sheet format as LP version)

CP-SAT formulation
------------------
Decision variables (only for eligible (SKU, machine) pairs):
    c[s,m]   : IntVar >=0    — number of cure cycles for SKU s on machine m
    y[s,m]   : BoolVar       — 1 iff SKU s assigned to machine m
    slack[s] : IntVar >=0    — unmet demand-units for SKU s

Constraints (all native integer in CP-SAT):
    Demand   :  CAVITIES * sum_m c[s,m] + slack[s] >= Demand[s]
    Capacity :  sum_s ct[s]*c[s,m] + co * sum_s y[s,m] <= avail[m] - locked[m] + co
                (encodes (n_active - 1) changeovers for n_active >= 1)
    Linking  :  c[s,m] > 0 iff y[s,m] == 1    (via OnlyEnforceIf)

Objective (weighted sum; slack dominates):
    minimise  W_SLACK * sum(slack[s])  +  sum(y[s,m])

Why CP-SAT
----------
* Native integer variables — no float/integer mixing or rounding loss
* Parallel search (num_search_workers) exploits multi-core CPUs
* Strong propagation reduces the search space via implication graph
* Handles combinatorial scheduling problems better than pure MILP
* Reified constraints (OnlyEnforceIf) express the linking rule cleanly

Dependency: pip install ortools

Input / output format is IDENTICAL to the LP variant — same CSVs/XLSX in,
same 5-sheet Excel out.
        ...
        for mach, sku_order in machine_sku_order.items():
            ...
            for idx, sku in enumerate(sku_order):
                ...
                prod_rows, units_so_far = self._split_block(
                    ...
                    gt, "CP-SAT Scheduled", units_so_far,
                )
...
# ORCHESTRATOR
# ══════════════════════════════════════════════════════════════════════════════
class JK_CPSAT_Curing_Scheduler_v1:
    def __init__(self):
        self.avail_mins = Config.avail_mins()
    ...
    def run(
        ...
        print("\n" + "="*64)
        print(f"  JK Tyre PCR Curing CP-SAT Scheduler v1")
        ...
        print(f"  CP-SAT      : time={Config.CPSAT_TIME_LIMIT_SEC}s | "
        ...
        # Reduce CP-SAT demand by what continuity already covers
        # SKUs fully handled in continuity are excluded from the solver entirely
        df_lp = df_valid.copy()
        for sku, remainder in demand_remainder.items():
            mask = df_lp["SKUCode"] == sku
            if remainder == 0:
                # Continuity covers all demand — remove from solver
                df_lp = df_lp[~mask]
            else:
                # Continuity covers part — solver only needs the remainder
        ...
        print(f"  [CP-SAT Input] {len(df_lp)} SKUs | "
              f"Remaining demand: {df_lp['Demand'].sum():,.0f} units")

        print("\n[Phase 3] Solving CP-SAT...")
        solver   = CPSAT_Solver()
        solution, meta = solver.solve(df_lp, all_machines, mould_tracker, locked_mins)

        print("\n[Phase 4] Extracting integer cycles (no rounding loss)...")
        extractor = CPSAT_Extractor()
        df_mach, machine_sku_order = extractor.extract(solution, meta, df_lp, locked_mins)
...
class ExcelExporter:
    ...
    def export(self, results: dict):
        ...
        with pd.ExcelWriter(self.path, engine="openpyxl") as writer:
            ...
            self._title(ws,"PCR CURING CP-SAT v1 — DEMAND FULFILLMENT",kpi,len(cols1))
            ...
            self._title(ws2,"MACHINE-WISE SCHEDULE — PCR CP-SAT v1 (CO + Cleaning)",kpi,len(cols2))
            ...
            self._title(ws3,"SHIFT-WISE SCHEDULE — PCR CP-SAT v1",kpi,len(cols3))
            ...
            self._title(ws4,"PRESS UTILIZATION — PCR CP-SAT v1",
...
def run_from_excel(
    ...
    scheduler = JK_CPSAT_Curing_Scheduler_v1()
