...
class Config:
    ...
    CHANGEOVER_DURATION_MIN = 360  # 240 min changeover + 60 min FTC check
    CLEANING_DURATION_MIN   = 180  # mould cleaning duration
    LOAD_UNLOAD_BUFFER_MIN  = 2.3  # added to raw cure time
    PRESS_EFFICIENCY        = 0.92  # divisor for warmup/transition losses
    DEAFULT_CYCLE_TIME= 59 

    # ── changeover scheduling ─────────────────────────────────────────────────
    MAX_CHANGEOVERS_PER_SHIFT = 100    # plant-level cap — configurable
...
# ══════════════════════════════════════════════════════════════════════════════
class ETL:
    def __init__(self, engine=None, tyre_type: str = "tbr"):
        self.engine = engine
        self.t = tyre_type
    ...
    def load_cycle_times(self) -> pd.DataFrame:
        df = self._sql(
            ...
            f"FROM {Config.DB_NAME}.Master_Curing_Design_CycleTime_tbr"
        )
    ...
    def load_machine_allowable(self) -> pd.DataFrame:
        df = self._sql(
            ...
            f".Master_Curing_Allowable_Machines_source_tbr"
        )
    ...
    def load_running_moulds(self) -> pd.DataFrame:
        ...
        df = self._sql(f"SELECT * FROM {Config.DB_NAME}.Daily_Running_Moulds_tbr")
...
class JK_LP_Curing_Scheduler_v2:
    ...
    def _prepare_skus(self, df_demand, df_cycles, df_allow, df_gt,
        ...
        for _, r in df_demand.iterrows():
            ...
            qty      = int(r["Quantity"])
            priority = r["Priority"]
            ct       = cycle_map.get(sku, Config.DEAFULT_CYCLE_TIME)
    ...
    def run(
        ...
        print("\n" + "="*64)
        print(f"  JK Tyre TBR Curing LP Scheduler v2")
...
class ExcelExporter:
    ...
    def export(self, results: dict):
        ...
        with pd.ExcelWriter(self.path, engine="openpyxl") as writer:
            ...
            self._title(ws,"TBR CURING v2 — DEMAND FULFILLMENT",kpi,len(cols1))
            ...
            self._title(ws2,"MACHINE-WISE SCHEDULE — TBR v2 (LP + CO + Cleaning)",kpi,len(cols2))
            ...
            self._title(ws3,"SHIFT-WISE SCHEDULE — TBR v2",kpi,len(cols3))
            ...
            self._title(ws4,"PRESS UTILIZATION — TBR v2",
...
# ══════════════════════════════════════════════════════════════════════════════
def run_from_excel(
    demand_path:  str = "Demand_for_Curing_Schedule3_tbr.xlsx",
    cycles_path:  str = "Master_Curing_Design_CycleTime_tbr.xlsx",
    allow_path:   str = "curing_tbr_machine_allowable.xlsx",
    gt_path:      str = "GT_Inventory_tbr.xlsx",
    mould_path:   str = "Master_Mapping_Mould_SKU.xlsx",
    running_path: str = None,
    plan_start:   datetime = None,
    output_path:  str = "TBR_Curing_LP_v2_Schedule.xlsx",
) -> dict:
...
'''if __name__ == "__main__":
    results = run_from_excel(
        demand_path  = "Demand_for_Curing_Schedule3_tbr.xlsx",
        cycles_path  = "Master_Curing_Design_CycleTime_tbr.xlsx",
        allow_path   = "curing_tbr_machine_allowable.xlsx",
        gt_path      = "GT_Inventory_tbr.xlsx",
        mould_path   = "Master_Mapping_Mould_SKU.xlsx",
        running_path = None,   # set to "Curing_Current_Running_moulds_tbr.xlsx" for continuity
        plan_start   = datetime(2026, 4, 1, 7, 0, 0),
        output_path  = "TBR_Curing_LP_v2_Schedule.xlsx",
    )'''
